...
\begin{figure}
  ...
  \caption{\small{\rqc{Fix latency at 5, bandwidth at $40$, Vary local memory from 0\% to 90\% of the total memory of the EC2 instance}}}
  \label{fig:impb}
\end{figure}
%
%
...
\label{ssec:rtt}
We discuss the feasibility of meeting the performance requirements identified above. Several datacenters are already deploying $40$Gbps links, with technology available even for $100$Gbps links. 

\cut{
% SR: notes to self. 
-- BW easy, latency hard. so let's look at components of latency and whether they can be accommodated in a budget of 5-10us.
-- first some unavoidable ones: (i) tx time: 0.8 (4KB div 40Gbps) and (ii) propagation delay RTT: 0.8us (0.6-1.2 reported in Mendel).
In addition, switch traversals: 0.2us per switch means 1us. So we've already consumed say 3us. Means we have about 2-7us left.
-- rack-scale would only improve this budget to 0.8 plus 0.05 plus 0.25 equals 1.1 hence budget 3.1-8.1us so unlikely to be a critical factor.
-- most important: low queuing. leaves little room for queueing (hence effective need near-zero queueing); important to note the significance of tx delay which we often ignore. E.g., queueing means we'll reincur the tx times at switches. e.g., just tx time at each switch would be  3.2us (budget -1.2-4); we'll examine the effect of 
congestion in S5.
-- also suggests a different role for high bw: not because we need high capacity but because going to 100Gbps decrease both tx and queueing time
-- turns out the elephant in the room are sw overheads. Already exceeds this range though recent efforts aiming to get this under 1us so we can be hopeful.
In summary: very tight but feasible.
}
