...
\begin{figure*}
  ...
  \subfigure[Flow size distribution (\dis)]{
    \includegraphics[width = 2.1in]{img/graph1_sizedist_dis} 
	\label{fig:fsd}
  }
%\hspace{0.05in}
  ...
  \subfigure[Temporal traffic distribution (\dis)]{
    \includegraphics[width = 2.1in]{img/graph7_temporaltraffic_dis_terasort}
	\label{fig:ttd}
  }  
  \subfigure[Flow size distribution (\pdis)]{
    \includegraphics[width = 2.1in]{img/graph1_sizedist_pdis}
	\label{fig:fsdp}
  }
  ...
  \subfigure[Temporal traffic distribution (\pdis)]{
    \includegraphics[width = 2.1in]{img/graph7_temporaltraffic_pdis_terasort}
	\label{fig:ttdp}
  }  
...
disk blade.\footnote{We arrived at this number \sr{how??}.}
...
Second, while the traffic volume in \dis is two to three orders of magnitude larger than in server-centric datacenters, the peak traffic does not change. This observation has the following implication:

\paragraphb{Implication \#6:} Increase in \dis network bandwidth capacity is likely to be guided by reduction in end-to-end network latency requirements rather than peak traffic volume.
...
\paragraphb{Spatial traffic distribution}
\rc{need to discuss this}
...
\label{ssec:knobs}
\rc{I don't think we'll have results for these, right?}
